...
function createPage(step) {
  ...
  const panels = [
    `<div class="form-grid"><div class="field"><label for="new-title">Song title</label><input id="new-title" name="title" placeholder="Untitled song" required></div><div class="field"><label for="new-artist">Artist</label><input id="new-artist" name="artist" placeholder="Artist name" required></div><div class="form-row"><div class="field"><label for="new-genre">Genre / blend</label><input id="new-genre" name="genre" placeholder="Genre or creative blend"></div><div class="field"><label for="new-stage">Current stage</label><select id="new-stage" name="stage"><option value="intent">Creative development</option><option value="lyrics">Lyrics</option><option value="chain">Vocal production</option></select></div></div></div>`,
...
function chainStep(project, step, key, index, mode) {
  ...
  return `<article class="card chain-step ${open ? "open" : ""}"><div class="chain-summary"><span class="chain-index">${index+1}</span><div class="chain-title"><div class="button-row"><h3>${escapeHtml(step.name)}</h3><span class="pill">${step.required ? "Required" : "Optional"}</span>${certainty(step.certainty)}</div><p>${escapeHtml(step.role)} · ${escapeHtml(step.plugin)}</p></div><div class="order-buttons"><button class="icon-button" data-action="move-step" data-project="${project.id}" data-step="${key}" data-direction="-1" aria-label="Move ${escapeHtml(step.name)} up">▲</button><button class="icon-button" data-action="move-step" data-project="${project.id}" data-step="${key}" data-direction="1" aria-label="Move ${escapeHtml(step.name)} down">▼</button></div><button class="icon-button" data-action="toggle-step" data-project="${project.id}" data-step="${key}" aria-expanded="${open}" aria-label="${open ? "Collapse" : "Expand"} ${escapeHtml(step.name)}">${open ? "▾" : "▸"}</button></div>${open ? `<div class="chain-detail"><div><div class="detail-label">Why it is here</div><p>${escapeHtml(step.why)}</p></div><div><div class="detail-label">Starting settings</div><div class="settings">${step.settings.map(([label,value]) => `<div class="setting"><strong>${escapeHtml(label)}</strong><span>${escapeHtml(value)}</span></div>`).join("")}</div></div>${mode === "advanced" ? `<div class="plugin-grid"><div class="plugin-box"><div class="section-label">Primary plugin</div>${escapeHtml(step.plugin)}</div><div class="plugin-box"><div class="section-label">Free / stock alternative</div><span style="color:var(--teal)">${escapeHtml(step.alternative)}</span></div></div>` : ""}<div class="listen-box"><span>🎧</span><div><div class="detail-label">Listen for</div><p>${escapeHtml(step.listen)}</p></div></div></div>` : ""}</article>`;
}
...
root.addEventListener("change", async event => {
  ...
  if (!file) return;
  const maximumAudioBytes = 150 * 1024 * 1024;
  if (file.type && !file.type.startsWith("audio/")) {
    alert("Choose a supported audio file.");
    event.target.value = "";
    return;
  }
  if (file.size > maximumAudioBytes) {
    alert("This audio file is larger than 150 MB. Export a shorter vocal section or a compressed listening copy, then try again.");
    event.target.value = "";
    return;
  }
