...
import { Play, Pause, Settings, Share, Upload, Mic } from 'lucide-react';
import cadenzaiLogo from '../assets/branding/Cadenzai_Logo_W.png';

export const TopBar: React.FC = () => {
  const { isPlaying, currentTime, duration, audioFileName, setIsImportModalOpen, setIsHumModalOpen } = useProjectStore();
  ...
  return (
    <div className="h-16 border-b border-zinc-800 bg-zinc-950 flex items-center justify-between px-6 shrink-0 relative z-50 shadow-sm">
      ...
      <div className="flex items-center justify-end gap-4 flex-1 text-zinc-400">
        <button onClick={() => setIsHumModalOpen(true)} className="hover:text-cyan-300 transition-colors p-2 rounded-md hover:bg-zinc-900 cursor-pointer" title="Hum to Spark"><Mic size={18} /></button>
